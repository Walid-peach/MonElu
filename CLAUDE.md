...
- `/votes/[id]` deliberately has **no `generateStaticParams`** (MON-275). Prerendering the 100 most recent votes meant ~200 API calls from one IP inside a build, which exhausted the API's DB connections and came back as 500s on votes that serve 200 on every manual request - two CI builds in a row died on a different vote each. `dynamicParams = true` plus `revalidate` already covers the route on demand. Do not reintroduce it without a way to throttle the burst.
