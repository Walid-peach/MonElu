...
// GET /agenda (MON-212, ADR-030) - the forward view. Every field is optional
// server-side to match the DB NULLs, so the client type mirrors that: an ODJ
// point can carry nothing but a uid and a slot.
export type AgendaItem = {
  ...
  // Generated by MON-211, and still null by design for stub objets - the
  // client must keep treating an absent one-liner as normal, not as an error.
  summary_plain: string | null
  theme: string | null
  dossier_id: string | null
  dossier_url: string | null
  // Set once a scrutin exists for this item's dossier: the item is settled,
  // not upcoming, and links to its MonÉlu vote page instead of the dossier.
  vote_id: string | null
  result: string | null
}
