...
Render the sub-issues as a dependency-ordered chain (numbered nodes, connector between blocked/blocking steps), each card carrying its title linked to the GitHub issue URL, one-line scope, and dependency note.
Use a status line noting how many sub-issues exist, how many are created, and the epic's current state.
If this report follows step 6 (issues actually created), link the real GitHub issue numbers; if the user has not yet approved creation, publish the draft plan instead and mark it as not yet filed - either way, keep the same file path across a single planning session so re-publishing after GitHub writes updates the same artifact URL instead of minting a new one.
